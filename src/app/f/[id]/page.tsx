        ...
        <div className="relative z-10 text-center px-6 max-w-lg w-full" data-testid="success-state">
          ...
          {scoreData?.isQuiz ? (
            <>
              ...
              <div data-testid="quiz-score" className="bg-background border-4 border-border shadow-retro inline-flex items-center justify-center px-8 py-4 mb-8 mt-4">
          ...
          <div className="bg-card border-4 border-border shadow-[8px_8px_0px_var(--border)] p-8 mb-8">
            {form.user_id && (
              <p className="font-sans font-medium text-foreground mb-8 text-lg border-y-4 border-dashed border-border/20 py-6">
                This form is secured in the vault. Results are available to the authorized owner.
              </p>
            )}
            {!form.user_id && (
              <div className="space-y-4 mb-8 border-y-4 border-dashed border-border/20 py-6">
                <p className="font-sans font-bold text-foreground text-lg">
                   <span className="font-pixel text-xl uppercase tracking-widest text-primary mr-2">Guest Form:</span> 
                   Sign in to save this form to your vault and manage it later.
                </p>
                <div className="flex flex-col gap-1">
                  <p className="font-pixel text-sm uppercase text-muted-foreground tracking-[0.2em]">
                    SECURE YOUR DATA
                  </p>
                  <p className="font-sans text-xs text-muted-foreground/60 italic">
                    (Otherwise, you must save this URL manually!)
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="flex flex-col gap-4">
              {form.user_id ? (
                ...
                <div className="flex flex-col gap-5">
                  <div className="flex flex-col gap-3">
                    <Button
                      onClick={() => router.push(`/login?next=/dashboard&claim_id=${formId}`)}
                      className="w-full h-14 inline-flex items-center justify-center gap-2 bg-primary text-primary-foreground border-4 border-border shadow-[4px_4px_0_var(--border)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0_var(--border)] active:translate-x-[4px] active:translate-y-[4px] active:shadow-none px-4 font-pixel text-lg uppercase tracking-wider transition-all"
                    >
                      Continue with Email
                    </Button>
                    <Button
                      ...
                      className="w-full h-14 inline-flex items-center justify-center gap-2 bg-card text-foreground border-4 border-border shadow-[4px_4px_0_var(--border)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0_var(--border)] active:translate-x-[4px] active:translate-y-[4px] active:shadow-none px-4 font-pixel text-lg uppercase tracking-wider transition-all"
                    >
              ...
              <div className="flex flex-col sm:flex-row gap-4 mt-8 pt-8 border-t-4 border-dashed border-border/10">
                <Link
                  href={form.user_id ? `/dashboard/${formId}` : `/dashboard/${formId}`} 
                  className="flex-1 h-14 inline-flex items-center justify-center gap-2 bg-secondary text-secondary-foreground border-4 border-border shadow-[4px_4px_0_var(--border)] hover:translate-x-px hover:translate-y-px hover:shadow-[2px_2px_0_var(--border)] px-4 font-pixel text-lg uppercase transition-all"
                >
                  Live Results
                </Link>
                <Link
                  href="/"
                  className="flex-1 h-14 inline-flex items-center justify-center gap-2 bg-card text-foreground border-4 border-border shadow-[4px_4px_0_var(--border)] hover:translate-x-px hover:translate-y-px hover:shadow-[2px_2px_0_var(--border)] px-4 font-pixel text-lg uppercase transition-all"
                >
                  Create Another
                </Link>
              </div>
