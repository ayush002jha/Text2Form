...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'LOGIN' button on the homepage to open the login page (element index 638).
        ...
        # -> Type the provided email into the Email field, type the provided password into the Password field, then click 'Sign In'. After that, verify the dashboard URL (this will be the next step after the click).
        frame = context.pages[-1]
        # Input text
        ...
        # -> Open the form editor for the first form by clicking its Edit button (use the dashboard card's Edit control). Then proceed to edit the title/description and save inside the editor.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[3]/main/div/div[2]/div/div[2]/div[3]/a[2]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the 'Save' button in the editor to save the updated title/description (element index 4284).
        frame = context.pages[-1]
        # Click element
