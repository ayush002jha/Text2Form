...
async def run_test():
    ...
    try:
        ...
        # -> Click the LOGIN button (index 84) to open the login page.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/header/div/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Type the email into the email field (index 966), type the password into the password field (index 970), then click the Sign In button (index 973).
        frame = context.pages[-1]
        # Input text
