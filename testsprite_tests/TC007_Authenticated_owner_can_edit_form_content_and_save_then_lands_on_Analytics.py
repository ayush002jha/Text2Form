...
async def run_test():
    ...
    try:
        ...
        # -> Click the LOGIN button (index 638) to go to the login page.
        ...
        # -> Fill the email and password fields with the provided credentials and click 'Sign In'.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Click the 'Edit Form' link for the first form to open the form editor (use the Edit link element). ASSERTION: Clicking Edit should open the editor page where 'Save' will be verifiable.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[3]/main/div/div[2]/div/div[2]/div[3]/a[2]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the 'My Forms' button (index 7093) to open the dashboard / forms list so the first form can be opened.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/header/div/div/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the 'Edit Form' link for the first form card (interactive element index 9786) to open the form editor so 'Save' can be verified.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[3]/main/div/div[2]/div/div[2]/div[3]/a[2]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Modify the form (change the Form Title) and then click the 'Save' button to trigger saving and redirect to the analytics page.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[3]/main/div/div[2]/div/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('Updated Form Title - autosave test')
